...
public class ZhuCacheManager {
    private static ZhuCacheManager mInstance = null;
    ...
    public static ZhuCacheManager getInstance(Context context) {
        return getInstance(context, Strategy.MEMORY_FIRST);
    }

    public static ZhuCacheManager getInstance(Context context, Strategy strategy) {
        if (mInstance == null) {
            synchronized (ZhuCacheManager.class) {
                if (mInstance == null) {
                    mInstance = new ZhuCacheManager(context.getApplicationContext(), strategy);
                }
            }
        } else {
    ...
    private ZhuCacheManager(Context context, Strategy strategy) {
        this.mStrategy = strategy;
        init(context);
    }

    public void setStrategy(ZhuCacheManager.Strategy strategy) {
        this.mStrategy = strategy;
        switch (mStrategy) {
            case MEMORY_FIRST:
